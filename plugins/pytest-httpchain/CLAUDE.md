...
- **Version bumps touch two files:** `.claude-plugin/plugin.json` and the matching `.claude-plugin/marketplace.json` entry (`name`/`description`/`version` in sync; `license` lives only in `marketplace.json`).
