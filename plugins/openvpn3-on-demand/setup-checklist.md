...
### 3. `uv` installed
- Run: `uv --version`
- PASS if it prints a version.
- FAIL → "`uv` isn't on `PATH`. It bootstraps and runs the MCP server's venv (`servers/openvpn3/scripts/launch.sh`); without it the `openvpn3` MCP server never starts and no `vpn_*` tool is available. Install it: https://docs.astral.sh/uv/ — then restart Claude Code."
